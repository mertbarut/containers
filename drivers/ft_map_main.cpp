...
/*   Updated: 2022/02/14 17:54:02 by mbarut           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int main()
{
	...
	m1.erase("GPU");
	ft::print_map(m1, "After erase: ");
	
	std::cout << "m.size() = " << m1.size() << '\n';
	
	ft::map<std::string, int> m2 = m1;
	ft::print_map(m1, "Copy constructed map: ");
		
	std::cout << std::boolalpha << "m1 and m2 are equivalent: " << (m1 == m2) << '\n';
	m2["PSU"] = 34;
	m2["Peripherals"] = 12;
	ft::print_map(m2, "Updated map: ");
	std::cout << std::boolalpha << "m1 and m2 are equivalent: " << (m1 == m2) << '\n';
		
	m1.swap(m2);
	ft::print_map(m2, "Swapped map: ");
